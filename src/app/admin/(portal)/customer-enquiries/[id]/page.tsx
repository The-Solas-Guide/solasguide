...
import { isOperationalRecordId } from "@/lib/admin/operational-cms";

export default async function OperationalDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  if (!isOperationalRecordId(id)) notFound();
  const result = await getOperationalRecord("customer-enquiries", id);
  if (!result.ok) throw new Error("This private record could not be loaded. Try again.");
  if (!result.data) notFound();
  return <OperationalEditor key={result.data.id} kind="customer-enquiries" record={result.data} />;
}
