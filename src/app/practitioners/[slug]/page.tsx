...
import { getPractitionerMetadata, getUnavailableMetadata } from "@/lib/practitioner-metadata";
...
export default async function PractitionerProfilePage({ params }: PractitionerPageProps) {
  const { slug } = await params;
  const result = await getPublishedPractitionerBySlug(slug);
  if (result.error) return <div className="mx-auto w-full max-w-[1440px] overflow-x-hidden px-3 py-3 md:px-5 md:py-5"><SiteHeader links={[{ label: "Why Solas", href: "/#why-solas" }, { label: "Recognition", href: "/#recognition" }, { label: "The Guide", href: "/practitioners" }]} /><main id="main-content" className="mt-3"><PractitionerProfileError /></main><SiteFooter /></div>;
