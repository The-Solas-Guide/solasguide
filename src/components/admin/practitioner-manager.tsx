...
export function PractitionerManager({ initialRecords }: { initialRecords: AdminPractitionerRecord[] }) {
  ...
  return <div className="mx-auto flex w-full min-w-0 max-w-7xl flex-col gap-6">
    ...
    <AdminTableShell data={pageRecords} columns={columns} getRowId={(row) => row.id} query={query} onQueryChange={(next) => dispatch({ type: "hydrate", state: next })} totalCount={filtered.length} hasNextPage={pageStart + pageSize < filtered.length} statusTabs={[{ value: "all", label: "Every record", count: counts.all }, { value: "draft", label: "Draft", count: counts.draft }, { value: "published", label: "Published", count: counts.published }, { value: "archived", label: "Archived", count: counts.archived }]} filters={[{ id: "featured", label: "Featured", options: [{ value: "featured", label: "Featured only" }, { value: "not-featured", label: "Not featured" }] }, { id: "term", label: "Taxonomy term", options: termOptions }]} rowActions={(record) => <div className="flex flex-wrap gap-1"><Button asChild variant="ghost"><Link href={`/admin/practitioners/${record.id}`}>Edit</Link></Button>{record.status === "published" && <Button type="button" variant="ghost" onClick={() => feature(record)} disabled={pending}>{record.featured_position ? "Remove featured" : "Feature"}</Button>}{record.status === "archived" ? <Button type="button" variant="ghost" onClick={() => restore(record)} disabled={pending}>Restore</Button> : <AdminArchiveDialog recordName={record.name} onArchive={() => archive(record)} disabled={pending} />}</div>} renderMobileCard={(record) => <div className="grid gap-2"><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-medium">{record.name}</p><p className="truncate text-xs text-muted-foreground">{record.slug}</p></div><StatusBadge status={getPractitionerLifecycle(record)} /></div><p className="text-sm text-muted-foreground">{record.featured_position ? `Featured position ${record.featured_position}` : "Not featured"}</p><p className="text-xs text-muted-foreground">Updated {formatAdminDate(record.updated_at)}</p></div>} />
