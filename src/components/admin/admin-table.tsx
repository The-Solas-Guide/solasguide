  ...
  return (
    <section data-testid="admin-table-shell" className="flex min-w-0 w-full flex-col gap-4 overflow-hidden">
      <div className="flex min-w-0 flex-col gap-3">
        ...
        {statusTabs.length > 0 && <Tabs value={query.status} onValueChange={(status) => update({ status, page: 1 })}><TabsList data-testid="admin-status-navigation" aria-label="Record status" className="max-w-full overflow-x-auto"><TabsTrigger value="all">{allTab?.label ?? "All"}{typeof allTab?.count === "number" && <Badge variant="outline" className="ml-1 min-h-5 px-1.5">{allTab.count}</Badge>}</TabsTrigger>{statusTabs.filter((tab) => tab.value !== "all").map((tab) => <TabsTrigger key={tab.value} value={tab.value}>{tab.label}{typeof tab.count === "number" && <Badge variant="outline" className="ml-1 min-h-5 px-1.5">{tab.count}</Badge>}</TabsTrigger>)}</TabsList></Tabs>}
      </div>
