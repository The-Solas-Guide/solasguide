...
export function TaxonomyManager({ initialRecords }: { initialRecords: AdminTaxonomyRecord[] }) {
  ...
  const columns = [
    ...
    { id: "state", header: "Status", enableSorting: true, cell: ({ row }: { row: { original: AdminTaxonomyRecord } }) => <StateBadge state={getTaxonomyLifecycle(row.original)} /> },
    { id: "usage", header: "Practitioners", enableSorting: true, cell: ({ row }: { row: { original: AdminTaxonomyRecord } }) => <div className="flex min-w-0 items-center gap-2"><span className="inline-flex size-8 shrink-0 items-center justify-center rounded-full bg-muted text-muted-foreground"><UsersIcon className="size-4" /></span><div className="min-w-0"><p className="font-medium tabular-nums">{row.original.usageCount}</p><p className="max-w-48 truncate text-xs text-muted-foreground">{row.original.practitioners.length > 0 ? row.original.practitioners.map((practitioner) => practitioner.name).join(", ") : "No linked practitioners"}</p></div></div> },
    { accessorKey: "sort_order", header: "Order", cell: ({ row }: { row: { original: AdminTaxonomyRecord } }) => <span className="tabular-nums text-muted-foreground">{row.original.sort_order}</span> },
