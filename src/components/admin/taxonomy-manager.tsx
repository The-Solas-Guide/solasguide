...
export function TaxonomyManager({ initialRecords }: { initialRecords: AdminTaxonomyRecord[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((record) => (query.status === "all" || getTaxonomyLifecycle(record) === query.status) && (!type || record.type === type) && (!search || `${record.name} ${record.slug}`.toLowerCase().includes(search))).sort((a, b) => a.sort_order - b.sort_order || a.name.localeCompare(b.name));
  }, [query, records]);
  ...
  const columns = [
    { accessorKey: "name", header: "Name", enableSorting: true, cell: ({ row }: { row: { original: AdminTaxonomyRecord } }) => <div><p className="font-medium">{row.original.name}</p><p className="text-xs text-muted-foreground">{row.original.slug}</p></div> },
    { accessorKey: "type", header: "Type", cell: ({ row }: { row: { original: AdminTaxonomyRecord } }) => <span>{taxonomyTypeLabel(row.original.type)}</span> },
    { id: "state", header: "State", cell: ({ row }: { row: { original: AdminTaxonomyRecord } }) => <StateBadge state={getTaxonomyLifecycle(row.original)} /> },
    { id: "usage", header: "Usage", cell: ({ row }: { row: { original: AdminTaxonomyRecord } }) => <div><span>{row.original.usageCount} practitioner{row.original.usageCount === 1 ? "" : "s"}</span>{row.original.practitioners.length > 0 && <ul className="mt-1 text-xs text-muted-foreground">{row.original.practitioners.slice(0, 3).map((practitioner) => <li key={practitioner.id}><Link className="underline" href={`/admin/practitioners/${practitioner.id}`}>{practitioner.name}</Link></li>)}</ul>}</div> },
    { accessorKey: "updated_at", header: "Updated", cell: ({ row }: { row: { original: AdminTaxonomyRecord } }) => <span className="text-sm text-muted-foreground">{formatAdminDate(row.original.updated_at)}</span> },
  ];
