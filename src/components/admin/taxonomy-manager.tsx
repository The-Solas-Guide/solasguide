...
function TypeBadge({ type }: { type: AdminTaxonomyRecord["type"] }) {
  return (
    <span className="inline-flex w-fit items-center rounded-full border border-border/80 bg-muted/35 px-2 py-1 text-xs font-medium text-foreground">
      {taxonomyTypeLabel(type)}
    </span>
  );
}
...
export function TaxonomyManager({ initialRecords }: { initialRecords: AdminTaxonomyRecord[] }) {
  ...
  const columns = [
    ...
    { accessorKey: "type", header: "Type", enableSorting: true, cell: ({ row }: { row: { original: AdminTaxonomyRecord } }) => <TypeBadge type={row.original.type} /> },
  ...
  return <AdminPage>
    <AdminPageHeader title="Taxonomy" description="Manage the shared terms that organise practitioner practices, locations, and languages." actions={<Button asChild><Link href="/admin/taxonomy/new"><PlusIcon />New term</Link></Button>} />
