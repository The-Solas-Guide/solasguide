...
export function TaxonomyManager({ initialRecords }: { initialRecords: AdminTaxonomyRecord[] }) {
  ...
  const columns = [
    ...
    { id: "usage", header: "Practitioners", enableSorting: true, cell: ({ row }: { row: { original: AdminTaxonomyRecord } }) => <span className="inline-flex items-center gap-2 whitespace-nowrap text-sm"><UsersIcon className="size-4 text-muted-foreground" /><span className="font-medium tabular-nums">{row.original.usageCount}</span><span className="text-muted-foreground">linked</span></span> },
