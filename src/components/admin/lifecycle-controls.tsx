...
function PublicLifecycleControls({ value, onChange, disabled = false, onArchive, recordName = "this record" }: PublicLifecycleControlsProps) {
  ...
  return <ControlGroup label="Public lifecycle"><div className="flex min-w-0 flex-1 flex-col gap-1"><span className="text-sm font-semibold">Public lifecycle</span><span className="text-sm text-muted-foreground">Publishing controls public visibility.</span></div><Select value={value} onValueChange={(next) => { if (next === "draft" || next === "published") onChange(next); }} disabled={disabled}><SelectTrigger aria-label="Public status" className="w-full sm:w-44"><SelectValue /></SelectTrigger><SelectContent>{value === "archived" && <SelectItem value="archived" disabled>Archived</SelectItem>}<SelectItem value="draft">Draft</SelectItem><SelectItem value="published">Published</SelectItem></SelectContent></Select><div className="flex w-full flex-wrap gap-2 sm:w-auto"><Button type="button" onClick={() => onChange(value === "archived" ? "draft" : value === "published" ? "draft" : "published")} disabled={disabled}>{value === "archived" ? "Restore" : value === "published" ? "Unpublish" : "Publish"}</Button>{value !== "archived" && <AdminArchiveDialog recordName={recordName} onArchive={archive} disabled={disabled} />}</div>{value === "draft" && <p className="w-full text-sm text-muted-foreground">Publishing makes this record visible on the public site.</p>}{value === "published" && <p className="w-full text-sm text-muted-foreground">Saving changes updates the public site.</p>}</ControlGroup>;
}
...
function TaxonomyLifecycleControls({ value, onChange, disabled = false, onArchive, recordName = "this record" }: TaxonomyLifecycleControlsProps) {
  const archive = () => onArchive ? onArchive() : onChange("archived");
  return <ControlGroup label="Taxonomy lifecycle"><div className="flex min-w-0 flex-1 flex-col gap-1"><span className="text-sm font-semibold">Taxonomy lifecycle</span><span className="text-sm text-muted-foreground">Activation controls where a value can be used.</span></div><Select value={value} onValueChange={(next) => { if (next === "active" || next === "inactive") onChange(next); }} disabled={disabled}><SelectTrigger aria-label="Taxonomy status" className="w-full sm:w-44"><SelectValue /></SelectTrigger><SelectContent>{value === "archived" && <SelectItem value="archived" disabled>Archived</SelectItem>}<SelectItem value="active">Active</SelectItem><SelectItem value="inactive">Inactive</SelectItem></SelectContent></Select><div className="flex w-full flex-wrap gap-2 sm:w-auto"><Button type="button" onClick={() => onChange(value === "archived" ? "inactive" : value === "active" ? "inactive" : "active")} disabled={disabled}>{value === "archived" ? "Restore" : value === "active" ? "Mark inactive" : "Activate"}</Button>{value !== "archived" && <AdminArchiveDialog recordName={recordName} onArchive={archive} disabled={disabled} />}</div></ControlGroup>;
}
