...
import { formatAdminDate, getPractitionerLifecycle, validatePortraitFile, type TaxonomyRow } from "@/lib/admin/practitioner-cms";
import { portraitObjectPosition } from "@/lib/practitioners";
import { archivePractitioner, deletePractitioner, savePractitioner, setPractitionerFeaturedPosition, type AdminPractitionerRecord } from "@/lib/admin/practitioner-actions";
...
type Props = { record: AdminPractitionerRecord | null; terms: TaxonomyRow[]; isNew?: boolean };

export function PractitionerEditor({ record, terms, isNew = false }: Props) {
  const router = useRouter();
  const [status, setStatus] = useState(record ? getPractitionerLifecycle(record) : "draft");
  const [selectedTerms, setSelectedTerms] = useState(() => new Set(record?.terms.map((term) => term.id) ?? []));
  ...
  const [featuredPosition, setFeaturedPosition] = useState(record?.featured_position ?? 1);
  const [isFeatured, setIsFeatured] = useState(record?.featured_position !== null);
  ...
  const updateFeatured = (next: number | null) => startTransition(async () => {
    if (!record) return;
  ...
  return <div className="mx-auto flex w-full min-w-0 max-w-4xl flex-col gap-6">
    ...
    <AdminFormLayout title={isNew ? "New practitioner" : `Edit ${record?.name ?? "practitioner"}`} description="Keep public profile details specific and current. Saving a published record updates the public site." status={status ? status[0].toUpperCase() + status.slice(1) : "Draft"} statusKind={status || "draft"} pending={pending} saved={saved} isDirty={dirty} error={error} validationErrors={fieldErrors} onSubmit={submit} onCancel={() => router.replace("/admin/practitioners")} saveLabel={isNew ? "Create practitioner" : "Save changes"}>
      ...
      <AdminFormSection title="Public profile" description="These fields make up the public practitioner record."><div className="grid gap-5">
        ...
        <AdminFormField name="slug" label="Slug" description="Use lowercase words separated by hyphens." error={fieldErrors.slug}><Input defaultValue={record?.slug ?? ""} required onChange={markDirty} /></AdminFormField>
        <AdminFormField name="descriptor" label="Descriptor"><Input defaultValue={record?.descriptor ?? ""} onChange={markDirty} /></AdminFormField>
        <div className="grid gap-5 sm:grid-cols-2"><AdminFormField name="yearsActive" label="Years active"><Input type="number" min="1" defaultValue={record?.years_active ?? ""} onChange={markDirty} /></AdminFormField><div className="grid gap-2"><span className="text-sm font-medium">Delivery</span><label className="flex min-h-11 items-center gap-2 text-sm"><input type="checkbox" name="offersInPerson" defaultChecked={record?.offers_in_person ?? true} onChange={markDirty} /> In-person</label><label className="flex min-h-11 items-center gap-2 text-sm"><input type="checkbox" name="offersOnline" defaultChecked={record?.offers_online ?? true} onChange={markDirty} /> Online</label></div></div>
        <AdminFormField name="summary" label="Summary" description="Required before publishing." error={fieldErrors.summary}><Textarea defaultValue={record?.summary ?? ""} required={status === "published"} onChange={markDirty} /></AdminFormField>
        <AdminFormField name="about" label="About" description="Required before publishing." error={fieldErrors.about}><Textarea defaultValue={record?.about ?? ""} required={status === "published"} onChange={markDirty} /></AdminFormField>
        <AdminFormField name="credentials" label="Credentials" description="One item per line or comma separated."><Textarea defaultValue={record?.credentials?.join("\n") ?? ""} onChange={markDirty} /></AdminFormField>
        <AdminFormField name="significantTraining" label="Significant training" description="One item per line or comma separated."><Textarea defaultValue={record?.significant_training?.join("\n") ?? ""} onChange={markDirty} /></AdminFormField>
