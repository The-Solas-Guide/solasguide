...
export function CustomerEnquiryForm() {
  ...
  return <main className="min-h-screen bg-muted/40 px-3 py-3 md:px-5 md:py-8"><div className="mx-auto w-full max-w-[1240px]"><div className="flex items-center justify-between gap-4 border-b border-border px-1 pb-5"><Link href="/" className="font-display text-2xl tracking-tight">The Solas Guide</Link><Link href="/" className="inline-flex items-center gap-2 text-xs uppercase tracking-[0.14em] text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" /> Back to the guide</Link></div>
    <section className="flex min-h-[700px] flex-col border-b border-border md:h-[min(920px,calc(100vh-128px))]"><div className="px-6 py-4 md:px-8 md:py-4"><h1 className="font-display text-2xl md:text-3xl">Tell us about your stay</h1><div className="mt-3" role="progressbar" aria-label="Enquiry progress" aria-valuemin={1} aria-valuemax={steps.length} aria-valuenow={index + 1}><div className="h-0.5 bg-muted"><div className="h-full bg-accent transition-[width]" style={{ width: `${((index + 1) / steps.length) * 100}%` }} /></div><div className="mt-2 flex justify-between text-[9px] uppercase tracking-[0.16em] text-muted-foreground"><span>Step {index + 1} of {steps.length}</span><span>{copy.eyebrow}</span></div></div></div>
