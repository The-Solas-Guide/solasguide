...
describe("practitioner admin actions", () => {
  ...
  it("returns a cleanup warning after replacing a portrait", async () => {
    ...
    const supabase = {
      from: vi.fn((table: string) => table === "practitioners" ? { select: vi.fn(() => practitionerQuery) } : { select: vi.fn(() => termQuery) }),
      storage: { from: vi.fn(() => storage) },
      rpc: vi.fn(async () => ({ data: adminId, error: null })),
    };
    ...
    expect(supabase.rpc).toHaveBeenCalledWith("save_admin_practitioner", expect.objectContaining({ p_image_path: expect.stringMatching(new RegExp(`^${adminId}/[0-9a-f-]{36}\\.jpg$`)), p_term_ids: [termId] }));
    vi.restoreAllMocks();
  });
  ...
  it("deletes the newly uploaded object when the database save fails", async () => {
    ...
    const supabase = {
      from: vi.fn((table: string) => table === "practitioners" ? { select: vi.fn(() => practitionerQuery) } : { select: vi.fn(() => termQuery) }),
      storage: { from: vi.fn(() => storage) },
      rpc: vi.fn(async () => ({ data: null, error: { message: "database rejected the update" } })),
    };
    mocks.createClient.mockResolvedValue(supabase);
