...
describe("taxonomy CMS helpers", () => {
  ...
  it("explains linked-term deletion blockers", () => {
    expect(taxonomyDeleteBlocker({ name: "Bali", usageCount: 2 })).toEqual({
      type: "Practitioner",
      name: "2 practitioner records",
      href: "/admin/practitioners?filter.term=bali",
      reason: "Remove the practitioner links before permanently deleting this term.",
    });
    expect(taxonomyDeleteBlocker({ name: "Bali", usageCount: 0 })).toBeNull();
  });
});
