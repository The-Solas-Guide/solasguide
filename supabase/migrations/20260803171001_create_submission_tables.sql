...
create table public.customer_enquiries (
  ...
  constraint customer_enquiries_full_name_check
    check (length(btrim(full_name)) between 1 and 200),
  constraint customer_enquiries_email_check
    check (length(email) between 3 and 320 and position('@' in email) > 1),
  constraint customer_enquiries_phone_check
    check (phone is null or length(phone) between 1 and 50),
  constraint customer_enquiries_contact_preference_check
    check (contact_preference in ('email', 'phone', 'whatsapp')),
  constraint customer_enquiries_contact_details_check
    check (contact_preference = 'email' or phone is not null),
  constraint customer_enquiries_consent_confirmed_check
    check (consent_confirmed),
  constraint customer_enquiries_answers_object_check
    check (
      jsonb_typeof(questionnaire_answers) = 'object'
      and pg_column_size(questionnaire_answers) <= 32768
    ),
  constraint customer_enquiries_source_check
    check (source in ('website')),
  constraint customer_enquiries_status_check
...
create index customer_enquiries_status_created_at_idx
  on public.customer_enquiries (status, created_at);
...
create trigger customer_enquiries_set_updated_at
...
-- Submission records are accepted only through a future validated server route.
...
grant all on table public.customer_enquiries to service_role;
...
comment on table public.customer_enquiries is
  'MVP customer enquiries. Access is restricted to the server-side service role.';
comment on table public.practitioner_expressions_of_interest is
  'MVP practitioner expressions of interest. Access is restricted to the server-side service role.';
comment on column public.customer_enquiries.internal_notes is
