...
select ok(
  to_regnamespace('admin_api') is null,
...
select ok(
  to_regprocedure('public.reorder_admin_featured(uuid[])') is not null,
  'featured reorder mutation is available in the public schema'
);
